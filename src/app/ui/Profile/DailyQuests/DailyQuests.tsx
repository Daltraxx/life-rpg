 ...
 * If either data fetch fails, a generic error message is displayed to the user.
 *
...
export default async function DailyQuests({ userId }: { userId: string }) {
  ...
  ] = await Promise.all([getDailyQuests(userId), getUsername(userId)]);
  if (questsError || usernameError) {
    return (
      <div>
  ...
  return (
    <div className={styles.container}>
      <Heading as="h1" color="brown-600" size="48-responsive">
        Welcome, {username || "quester"}.
      </Heading>
